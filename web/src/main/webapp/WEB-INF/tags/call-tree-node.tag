    ...
    <div class="node-row node-row-${node.type}">
        <c:choose>
            <c:when test="${node.duration.total > 0}">
                <c:set var="durationPercentage"><fmt:formatNumber value="${node.duration.percentage * 100}"
                                                                  maxIntegerDigits="3" maxFractionDigits="0"/></c:set>
                <c:set var="childrenDurationPercentage"><fmt:formatNumber
                        value="${node.duration.children * durationPercentage / node.duration.total}"
                        maxIntegerDigits="3" maxFractionDigits="0"/></c:set>
            </c:when>
            <c:otherwise>
        ...
        <div class="node-duration-wrapper">
            <div class="duration-box" style="width: ${durationPercentage}px"
                 title="Self time: ${node.duration.self}ms, internal calls: ${node.duration.children}ms">
                <div class="duration-box-children" style="width: ${childrenDurationPercentage}px"></div>
                <div class="duration-box-self"></div>
            </div>
            <div class="duration-text">${durationPercentage}% - <fmt:formatNumber value="${node.duration.total}"/>ms
            </div>
        </div>
